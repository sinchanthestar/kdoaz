...
local function hookParryRemote()
    for _, connection in pairs(getconnections(PlayerGui.Hotbar.Block.Activated)) do
        if connection.Function and not iscclosure(connection.Function) then
            local upvalues = getupvalues(connection.Function)
            for _, upvalue in pairs(upvalues) do
                if type(upvalue) == "function" then
                    local constants = getconstants(upvalue)
                    for _, constant in pairs(constants) do
                        if type(constant) == "string" and constant:find("ParryButtonPress") then
                            local oldFunc = upvalue
                            local newFunc = function(...)
                                local args = {...}
                                if #args >= 4 and typeof(args[4]) == "CFrame" then
                                    revertedRemotes.lastArgs = args
                                end
                                return oldFunc(...)
                            end
                            setupvalue(connection.Function, _, newFunc)
                        end
                    end
                end
            end
...
local function ParryV2()
    ...
    if revertedRemotes.lastArgs then
        task.wait(0.05)
        local direction = GetDirectionV2()
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if hrp then
            revertedRemotes.lastArgs[4] = CFrame.new(hrp.Position, hrp.Position + direction)
            game:GetService("ReplicatedStorage").Remotes.ParryButtonPress:Fire(unpack(revertedRemotes.lastArgs))
        end
    end
end
...
local function AutoAbilityV2()
    ...
    if abilityCD.Offset.Y == 0.5 then
        ...
        if char and char:FindFirstChild("Abilities") then
            ...
            if abilities:FindFirstChild("Raging Deflection") and abilities["Raging Deflection"].Enabled then hasAbility = true end
            if abilities:FindFirstChild("Rapture") and abilities["Rapture"].Enabled then hasAbility = true end
            if abilities:FindFirstChild("Calming Deflection") and abilities["Calming Deflection"].Enabled then hasAbility = true end
            if abilities:FindFirstChild("Aerodynamic Slash") and abilities["Aerodynamic Slash"].Enabled then hasAbility = true end
            if abilities:FindFirstChild("Fracture") and abilities["Fracture"].Enabled then hasAbility = true end
            if abilities:FindFirstChild("Death Slash") and abilities["Death Slash"].Enabled then hasAbility = true end
...
local Tabs = {
    ...
    Spam = Window:AddTab({ Name = "Spam", Icon = "rbxassetid://6031094678" }),
